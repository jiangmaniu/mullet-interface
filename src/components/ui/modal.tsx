...
import { Drawer, DrawerClose, DrawerContent, DrawerDescription, DrawerOverlay, DrawerPortal, DrawerTitle } from './drawer'
...
const ModalContent = ({ className, children, ...props }: ComponentProps<typeof DialogContent | typeof DrawerContent>) => {
  const isDesktop = useMediaQuery('(min-width: 768px)')
      ...
      <ModalPortal>
        ...
        <DialogContent
          className={cn(
            ...
            'data-[state=open]:animate-in data-[state=open]:fade-in-0 data-[state=open]:zoom-in-95 data-[state=open]:slide-in-from-left-[52%]',
            'data-[state=closed]:animate-out data-[state=closed]:fade-out-0 data-[state=closed]:zoom-out-95 data-[state=closed]:slide-out-to-top-[52%]',
            className
          )}
          {...props}
        >
...
function ModalOverviewItem({ className, ...props }: React.HTMLAttributes<HTMLDivElement> & { label?: React.ReactNode }) {
  return (
...
function ModalFooter({ className, ...props }: React.HTMLAttributes<HTMLDivElement>) {
  return <div className={cn('flex gap-3 py-8', className)} {...props} />
}
...
const ModalDescription = ({ className, ...props }: ComponentProps<typeof DialogDescription | typeof DrawerDescription>) => {
  const isDesktop = useMediaQuery('(min-width: 768px)')
