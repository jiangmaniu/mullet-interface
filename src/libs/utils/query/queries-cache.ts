...
export function buildQueriesCacheKey<T extends QueriesKeyConfig, P extends readonly string[]>(
  keyConfig: T,
  prefix: P
): CacheKeyHelperType<T, P>

export function buildQueriesCacheKey<T extends QueriesKeyConfig, P extends readonly string[]>(
  keyConfig: T,
  prefix: P = [] as const as unknown as P
): CacheKeyHelperType<T, P> {
  ...
  for (const k of Object.keys(keyConfig)) {
    ...
    if (typeof v === 'function') {
      ...
      toolObj[k] = {
        toKey: () => keyFn(k as string),
        ...buildQueriesCacheKey(v as QueriesKeyConfig, keyFn(k as string))
      } as unknown as QueriesKeyConfig[keyof QueriesKeyConfig]
    } else {
      toolObj[k] = {
        toKey: () => keyFn(k as string)
