export default {
  // ========== 登录模块 开始 ============
  'mt.denglu': '登录',
  'mt.dengluzhong': '登录中',
  'mt.loginTips': '提示：交易过程中可以关注更多信息面参考',
  'mt.wangjimima': '忘记密码',
  'mt.zhucechenggong': '注册成功',
  'mt.lijizhuce': '立即注册',
  'mt.zhucetips':
    '點擊“立即注冊”，即表示您同意<span style="color:#051C2C;font-weight:bold">服務條款</span>和<span style="color:#051C2C;font-weight:bold">隱私政策</span>',
  'mt.kailixinzhanghu': '开立新账户',
  'mt.shurumima': '输入密码',
  'mt.qingshurumima': '请输入密码',
  'mt.pleaseInputPwdPlaceholder': '請輸入8-16位字母+數字+特殊字符組合的密碼',
  'mt.tuxingyanzhengma': '图形验证码',
  'mt.shuruyanzhengma': '输入验证码',
  'mt.youxianggeshibuzhengque': '邮箱格式不正确',
  'mt.shurudianziyouxiang': '输入电子邮箱',
  'mt.qingxuanzequhao': '请选择区号',
  'mt.qingshuruyanzhengma': '请输入验证码',
  'mt.shezhixinmima': '设置新密码',
  'mt.mimaxiugaiwancheng': '密码修改完成',
  'mt.qingjizhumimabuyaoxielu': '请记住密码，不可向他人泄露',
  'mt.chongzhimima': '重置密码',
  'mt.fasongyanzhengma': '发送验证码',
  'mt.qudenglu': '去登录',
  'mt.dengluchenggong': '登录成功',
  // ========== 登录模块 结束 ============

  // ======= 交易页面 开始 ==========
  'mt.qiehuanzhanghuzhong': '切换账户中',
  'mt.shijiadan': '市价单',
  'mt.xianjiadan': '限價單',
  'mt.tingsundan': '停損單',
  'mt.maichuzuokong': '賣單·開空',
  'mt.mairuzuoduo': '買單·開多',
  'mt.kaikong': '開空',
  'mt.kaiduo': '開多',
  'mt.gangganbeishu': '杠杆倍数',
  'mt.tiaozhengganggan': '调整杠杆',
  'mt.tiaozhengganggantip1': '当前杠杆倍数最高可开:100,000,000 USDT',
  'mt.tiaozhengganggantip2': '杠杆调整将同时影响当前仓位和挂单的杠杆',
  'mt.tiaozhengganggantip3': '选择超过[10x]杠杆交易会增加强行平仓风险，请注意相关风险',
  'mt.quancang': '全仓',
  'mt.keyong': '可用',
  'mt.zijinhuilvdaojishi': '资金汇率/倒计时',
  'mt.weicangwei': '无仓位',
  'mt.baozhengjinlv': '保证金率',
  'mt.weichibaozhengjin': '维持保证金',
  'mt.mairushoushu': '买入手数',
  'mt.maichushoushu': '卖出手数',
  'mt.gudingyufukuan': '固定预付款',
  'mt.yuguqiangpingjia': '预估强平价',
  'mt.kekai': '可开',
  'mt.baozhengjinmoshi': '保证金模式',
  'mt.baozhengjinmoshitips': '调整保证金模式仅对当前标的生效',
  'mt..baozhengjinmoshitips': '调整保证金模式仅对当前标的生效',
  'mt.quancangdesc': '所有仓位共用保证金，盈亏互抵。抵御强平风险，但可能损失全部仓位',
  'mt.zhucangdesc': '各仓位的保证金与盈亏单独核算。强平事件中只会损失当前仓位',
  'mt.pinlei': '品类',
  'mt.zuixinjiage': '最新价格',
  'mt.zhucang': '逐仓',
  'mt.kucunfei': '库存费',
  'mt.shouyilv': '收益率',
  'mt.qiangpingjia': '強平價',
  'mt.biaozhijia': '標記價',
  'mt.quanbupingcang': '全部平倉',
  'mt.qingshurujiage': '请输入价格',
  'mt.shurujiage': '輸入價格',
  'mt.qingshurushoushu': '请输入手数',
  'mt.shousushuruyouwu': '手数输入有误',
  'mt.zhiyingzhisunshezhicuowu': '止盈止損設置錯誤',
  'mt.mairujiafanwei': '買入價範圍',
  'mt.maichujiafanwei': '賣出價範圍',
  'mt.yongxu': '永續',
  'mt.zhenshijiaoyi': '真實交易',
  'mt.monijiaoyi': '模擬交易',
  'mt.jiage': '价格',
  'mt.shezhiyingzhisun': '設置止盈止損',
  'mt.heyueshuxing': '合約屬性',
  'mt.zixuan': '自選',
  'mt.mingcheng': '名稱',
  'mt.zuixinjia': '最新價',
  'mt.leixing': '类型',
  'mt.zhangdiefu': '漲跌幅',
  'mt.ganggan': '杠桿',
  'mt.shoushufanwei': '手數範圍',
  'mt.zhisun': '止損',
  'mt.fanwei': '範圍',
  'mt.yujiyingkui': '預計盈虧',
  'mt.zhiying': '止盈',
  'mt.heyuejiazhi': '合約價值',
  'mt.cankaobaozhengjin': '參考保證金',
  'mt.pingcangchenggong': '平倉成功',
  'mt.jiancangchenggong': '開倉成功',
  'mt.guadanchenggong': '掛單成功',
  'mt.chicangdanhao': '持倉單號',
  'mt.chicangchanpin': '持倉產品',
  'mt.chicangfangxiang': '持倉方向',
  'mt.pingcangshoushu': '平倉手數',
  'mt.kaicangshoushu': '開倉手數',
  'mt.pingcangjiage': '平倉價格',
  'mt.kaicangjiage': '開倉均价',
  'mt.guadanchengjia': '掛單價格',
  'mt.geyelixi': '隔夜利息',
  'mt.pingcang': '平倉',
  'mt.chanpinmingcheng': '產品名稱',
  'mt.fudongyingkui': '浮動盈虧',
  'mt.kepingcangshoushu': '可平倉手數',
  'mt.quedingpingcang': '確定平倉',
  'mt.dangqianjiage': '當前價格',
  'mt.xiugaiguadan': '修改掛單',
  'mt.guadanfangxiang': '掛單方向',
  'mt.xianjiaguadan': '限價掛單',
  'mt.guadanshoushu': '掛單手數',
  'mt.heyuedanwei': '合約單位',
  'mt.huobidanwei': '貨幣單位',
  'mt.baojiaxiaoshuwei': '報價小數位',
  'mt.danbijiaoyishoushu': '單筆交易手數',
  'mt.shoushuchazhi': '手數差值',
  'mt.geyelixiduodan': '隔夜利息（多單）',
  'mt.geyelixikongdan': '隔夜利息（空單）',
  'mt.xianjiahetingsunjuli': '限價和停損距離',
  'mt.baozhengjin': '保證金',
  'mt.chushibaozhengjin': '初始保證金',
  'mt.suocangbaozhengjin': '鎖倉保證金',
  'mt.xingqiri': '週日',
  'mt.xingqiyi': '週一',
  'mt.xingqier': '週二',
  'mt.xingqisan': '週三',
  'mt.xingqisi': '週四',
  'mt.xingqiwu': '週五',
  'mt.xingqiliu': '週六',
  'mt.jiaoyishijian': '交易时间',
  'mt.fangxiang': '方向',
  'mt.shuliangshou': '數量（手）',
  'mt.kaicangjia': '开仓均价',
  'mt.xianjia': '現價',
  'mt.zhiyingzhisun': '止盈止損',
  'mt.guadanhao': '掛單號',
  'mt.guadanjia': '掛單價',
  'mt.guadanshijian': '掛單時間',
  'mt.cexiao': '撤銷',
  'mt.chengjiaodanhao': '成交單號',
  'mt.pingcangjia': '平仓價',
  'mt.chengjiaoshijian': '成交時間',
  'mt.maichupingcang': '賣出平倉',
  'mt.mairupingcang': '買入平倉',
  'mt.chicang': '持倉',
  'mt.baozhengjinleix': '保证金类型',
  'mt.guadan': '掛單',
  'mt.dangrichengjiao': '當日成交',
  'mt.cexiaoguadan': '撤銷掛單',
  'mt.kaicang': '開倉',
  'mt.yingkui': '盈虧',
  'mt.shuliang': '數量',
  'mt.jine': '金額',
  'mt.shijian': '時間',
  'mt.chongzhi': '重置',
  'mt.chengjiaoxiangqing': '成交詳情',
  'mt.shoushu': '手數',
  'mt.lot': '手',
  'mt.querenmairu': '確認買入',
  'mt.querenmaichu': '確認賣出',
  'mt.yidangqianzuixinjia': '以當前最優價',
  'mt.xuanzezhanghu': '选择账户',
  'mt.danhao': '单号',
  'mt.shifoucexiaogaibidingdan': '是否撤銷該筆訂單',
  'mt.zongzichanjingzhi': '總資產凈值',
  'mt.shuzihuobi': '數字貨幣',
  'mt.shangpin': '商品',
  'mt.waihui': '外匯',
  'mt.bid': '卖价',
  'mt.ask': '买价',
  'mt.zhishu': '指數',
  'mt.brand': '品牌',
  'mt.zhizhanshidangqian': '只展示當前',
  'mt.tradeTime': '交易時間',
  'mt.shouqudanbianzuida': '收取单边最大',
  'mt.xiazai': '下载',
  'mt.bunengshuruhanzi': '不能输入汉字',
  'mt.zaixiankefu': '在線客服',
  'mt.chart': '圖表',
  'mt.kaipanjiage': '開盤價格',
  'mt.shoupanjiage': '收盤價格',
  'mt.kaipan': '開盤',
  'mt.shoupan': '收盤',
  'mt.zuigao': '最高',
  'mt.zuidi': '最低',
  'mt.jiaqibaozhengjin': '假期保證金',
  'mt.pinpai': '品牌',
  'mt.lixi': '利息',
  'mt.shouxufei': '手續費',
  'mt.lishichengjiao': '歷史成交',
  'mt.dingdanhao': '訂單號',
  'mt.zijinbuzu': '資金不足',
  'mt.qurujin': '去入金',
  'mt.nidezijinbuzuqingrujin': '您的资金不足，请及时入金',
  'mt.guadanjiage': '挂单价格',
  'mt.kaicangchenggong': '開倉成功',
  'mt.zuixiaopingcangshoushuwei0.01shou': '最小平倉手數為0.01手',
  'mt.tixian': '提现',
  'mt.shijia': '市价',
  'mt.zengjin': '赠金',
  'mt.biandongqian': '变动前',
  'mt.dengluzhanghao': '登录账号',
  'mt.jiancang': '建仓',
  'mt.jiaoyizhanghao': '交易账号',
  'mt.symbol': '品种',
  'mt.jiaoyiliang': '交易量',
  'mt.xiugaishijian': '修改时间',
  'mt.dingdanleixing': '订单类型',
  'mt.beizhu': '备注',
  'mt.yue': '余额',
  'mt.jieyu': '结余',
  'mt.qiangping': '强平',
  'mt.lishiguadan': '历史挂单',
  'mt.lishiweituo': '历史委托',
  'mt.zijinliushui': '资金流水',
  'mt.wu': '无',
  'mt.zhong': '中',
  'mt.chengjiaoe': '成交额',
  'mt.tianjiazixuan': '添加自选',
  'mt.zhenshizhanghao': '真实账户',
  'mt.monizhanghu': '模拟账户',
  'mt.guanlizhanghu': '管理账户',
  'mt.zhenshi': '真实',
  'mt.moni': '模拟',
  'mt.zhiyingdan': '止盈单',
  'mt.zhisundan': '止损单',
  'mt.zhuijiabaozhengjin': '追加保证金',
  'mt.zhanyong': '占用',
  'mt.zichanjingzhitips': '立即關閉所有當前未結頭寸的新餘額估值',
  'mt.fudongyingkuitips': '當上倉位的浮動收益',
  'mt.keyongtips': '可用於開立新頭寸的資金',
  'mt.zhanyongtips': '我們為維持您當前持倉持有的資金',
  'mt.tiqubaozhengjin': '提取保证金',
  'mt.xianjiamairudan': '限价买入单',
  'mt.xianjiamaichudan': '限价卖出单',
  'mt.zhiyunxianjiamairudan': '止损限价买入单',
  'mt.zhiyunxianjiamaichudan': '止损限价卖出单',
  'mt.weituozhong': '委托中',
  'mt.shibai': '失败',
  'mt.yichengjiao': '已成交',
  'mt.yicexiao': '已撤销',
  'mt.daicexiao': '待撤销',
  'mt.bufenchengjiao': '部分成交',
  'mt.daichengjiao': '待成交',
  'mt.yijujue': '已拒绝',
  'mt.chicangzhong': '持仓中',
  'mt.yiwancheng': '已完成',
  'mt.xiugaizhiyingzhisunchenggong': '修改止盈止损成功',
  'mt.xiugaiguadanchenggong': '修改挂单成功',
  'mt.quxiaoguadanchenggong': '取消挂单成功',
  'mt.cexiaochenggong': '撤销成功',
  'mt.chengjiaojia': '成交价',
  'mt.24xiaoshizuigao': '24小时最高',
  'mt.24xiaoshizuidi': '24小时最低',
  'mt.zhiyingzhisun2': '止盈 / 止损',
  'mt.fudongyingkui_shouyilv': '浮动盈亏(USD)/收益率',
  'mt.biaojijia': '标记价',
  'mt.tianjia': '添加',
  'mt.baozhengjinbili': '保证金比例',
  'mt.yugubaozhengjin': '预估保证金',
  'mt.Crypto': '加密货币',
  'mt.Commodities': '大宗商品',
  'mt.Forex': '外汇',
  'mt.Indices': '指数',
  'mt.Stock': '股票',
  'mt.shoushushuruyouwu': '手數輸入有誤',
  'mt.gao': '高',
  'mt.di': '低',
  'mt.kai': '開',
  'mt.shou': '收',
  'mt.shou2': '手',
  'mt.mingxixiangqing': '明細詳情',
  'mt.qingshurubaozhengjin': '请输入保证金',
  'mt.zengjiazhengjin': '增加保证金',
  'mt.jianshaobaozhengjin': '减少保证金',
  'mt.zuiduokezengjia': '最多可增加',
  'mt.zuiduokejianshao': '最多可减少',
  'mt.zengjiahoudeyuguqiangpingjiagewei': '增加后的预估强平价格为',
  'mt.jianshaohoudeyuguqiangpingjiagewei': '减少后的预估强平价格为',
  'mt.lianjiezhengchang': '连接正常',
  'mt.duankailianjie': '断开连接',
  'mt.lianjiezhong': '连接中',
  'mt.hangqinglianjiezhongtips': '行情连接中...',
  'mt.hangqingyiduankaitips': '行情连接已断开，请检查您的网络连接是否正常',
  'mt.hangqinglianjiezhengchengtips': '行情连接正常稳定',

  'mt.qingqiujiage': '请求价格',
  'mt.mairu': '买入',
  'mt.maichu': '卖出',
  'mt.zhanghuyueTips': '未计入当前未结头寸的资金数目',
  'mt.lianjieguanbizhong': '连接关闭中',
  'mt.hangqinglianjieguanbizhong': '行情连接正在关闭中...',
  'mt.dangqianfuwumeiyouhangqingshujutuisong': '当前服务没有行情数据推送',
  'mt.zanwuhangqing': '暂无行情',
  'mt.xiushizhong': '休市中',
  'mt.zhanghubeijinyong': '账户被禁用',
  'mt.dangqianzhanghuyuebuzu': '当前账户余额不足',
  'mt.tiaozhengganggantips': '当前杠杆倍数最高可开仓位: {count}',
  'mt.zhiyunshijiamairudan': '止损市价买入单',
  'mt.zhiyunshijiamaichudan': '止损市价卖出单',

  'mt.zonge': '总额',
  'mt.zuiduokezengjiayue': '最多可增加约',
  'mt.zuiduokejianshaoyue': '最多可减少约',
  'mt.tiaozhengganggantip': '当前杠杆倍数最高可开: {count}',
  'mt.bujisuanweishixiandyinglikuyun': '不计算未实现的盈利/亏损',
  'mt.jisuanweishixiandyinglikuyun': '计算未实现的盈利/亏损',
  'mt.guiling': '归零',
  'mt.mairuchenggong': '买入成功',
  'mt.maichuchenggong': '卖出成功',
  'mt.chengjiao': '成交',
  'mt.dingdanchengjiao': '订单成交',
  // ======= 交易页面 结束 ==========

  // ======= 个人中心-我的账户 开始 ==========
  'mt.myAccount': '我的账户',
  'mt.yirenzheng': '已认证',
  'mt.weirenzheng': '未认证',
  'mt.chuangjianxinzhanghu': '创建新账户',
  'mt.chuangjianzhanghu': '创建账户',
  'mt.gerenzhongxin': '个人中心',
  'mt.quanbushoukuanfangshi': '全部收款方式',
  'mt.rujin': '入金',
  'mt.chujin': '出金',
  'mt.shezhi': '设置',
  'mt.churujinjilu': '出入金记录',
  'mt.qingshurushuzi': '請輸入數字',
  'mt.qingshuruzhengshu': '请输入正数',
  'mt.qingshuruzhengzhengshu': '请输入正整数',
  'mt.zhanghuchongmingming': '账户重命名',
  'mt.genggaijiaoyimima': '更改交易密码',
  'mt.cunruzijinkaishijiaoyi': '存入資金，開始交易',
  'mt.shezhizhanghumingcheng': '设置账户名称',
  'mt.shuruzhanghumingcheng': '输入账户名称',
  'mt.shuruzhanghumima': '输入账户密码',
  'mt.shezhizhanghumima': '设置账户密码',
  'mt.chuangjianchanghuchenggong': '创建账户成功',
  'mt.chuangjianzhanghuzhong': '创建账户中',
  'mt.xinmingcheng': '新名称',
  'mt.zhanghuchongmingmingtips': '如果您有多個帳戶，您可以以不同名稱命名每個帳戶以便區分。',
  'mt.zhanghu': '账户',
  'mt.guding': '固定',
  'mt.huazhuan': '划转',
  'mt.zhuanchuzhanghao': '转出账号',
  'mt.zhuanruzhanghao': '转入账号',
  'mt.yuebuzu': '余额不足',
  'mt.qingshuruyue': '请输入余额',
  'mt.xuanzezhuanchuzhanghao': '选择转出账号',
  'mt.xuanzezhuanruzhanghao': '选择转入账号',
  'mt.zhuanruzhanchuzhanghaobunengxiangtong': '转入转出账号不能相同',
  'mt.monizhanghurujin': '模拟账户入金',
  'mt.querenrujin': '确认入金',
  'mt.meirikerujin': '每日可入金',
  'mt.monizhanghurujinchenggong': '模拟账户入金成功',
  'mt.zhuanzhangxuzhi': '转账须知',
  'mt.zhuanzhangtip1': '1.出金金额少於50U，收取3U手續費',
  'mt.zhuanzhangtip2': '2.用戶交易量少於出金金额50%，收取出金金额5%手續費',
  'mt.zhuanzhangtip3': '3.當前有處理中的訂單時，無法出金',
  'mt.zhuanzhangtip4': '4.出金時間（GMT+8)：工作日09:00-18:00，不在此時段提交的出金訂單，會在下一工作日處理',
  'mt.zhuanzhangtip5': '5.請務必確認設備安全，防止信息被篡改或洩漏',
  'mt.zhifufangshi': '支付方式',
  'mt.zichizhanghujianzhuanzhang': '自持账户间转账',
  'mt.daichujinjine': '待出金金额',
  'mt.nindezijinjijiangdaozhang': '您的资金即将到账',
  'mt.zhuanzhangjieguo': '转账结果',
  'mt.jiaoyiyichuliwancheng': '交易已处理完成',
  'mt.shenqingchenggong': '申请成功',
  'mt.mianshouxufei': '免手续费',
  'mt.accountfudongyingkuitips': '当前仓位的浮动盈亏已包含相应的手续费和库存费',
  // ======= 个人中心-我的账户 结束 ==========

  // ======= 个人中心-设置 开始 ==========
  'mt.qingxianwanshankycrenzheng': '请先完成KYC认证',
  'mt.qurenzheng': '去认证',
  'mt.shurushoujihaoma': '输入手机号码',
  'mt.shurudianhua': '輸入手机号码',
  'mt.shoujihaobuzhengque': '手机不正确',
  'mt.bind': '绑定',
  'mt.xuanzequhao': '选择区号',
  'mt.tiqu': '提取',
  'mt.xuanzebizhong': '选择币种',
  'mt.daishenhe': '待审核',
  'mt.quxiao': '取消',
  'mt.shenheshibai': '审核失败',
  'mt.shenfenzheng': '身份证',
  'mt.huzhao': '护照',
  'mt.chuxuka': '储蓄卡',
  'mt.xingyongka': '信用卡',
  'mt.xuanzeguojia': '选择国家',
  'mt.juzhuguojiadiqu': '居住国家',
  'mt.fengxianjinggao': '風險警告',
  'mt.chakanyouxianghuoquyanzhengma': '查看邮箱，获取验证码',
  'mt.weishoudaoyanzhengma': '未收到验证码？',
  'mt.qingzaixxmiaohouchongshi': '请在{count}秒后重试',
  'mt.yanzhengmafasongzhi': '验证码发送至{email},请查看您的收件箱和垃圾邮箱',
  'mt.chakanshoujihuoquyanzhengma': '查看手机，获取验证码',
  'mt.yanzhengmayifasongzhixx': '验证码发送至{value}',
  'mt.anquanshezhi': '安全设置',
  'mt.chujindizhi': '出金地址',
  'mt.yanzhengzhuangtai': '验证状态',
  'mt.yiwanchengxxstep': '已完成 {step} / 2步',
  'mt.rujinxianzhi': '入金限制',
  'mt.wuxian': '无限',
  'mt.qujueyuzhifufangshi': '取决于支付方式',
  'mt.zanweiwancheng': '暂未完成',
  'mt.yanzhengbuzhou': '验证步骤',
  'mt.zhanghuguanli': '账户管理',
  'mt.zhanghuguanlitips': '用于登录{name}的信息。如您认为密码可能已经泄露，请随时更改密码',
  'mt.anquanrenzhengfangshi': '安全验证方式',
  'mt.anquanrenzhengfangshitips': '安全验证可以确保所有敏感交易均已得到您的授权我们建议您输入验证码确认相关交易',
  'mt.anquanleixing': '安全类型',
  'mt.yiquerengerenziliao': '已确认个人资料',
  'mt.ninyiquerengerenziliao': '您已确认个人资料',
  'mt.nindeshenfenyirenzheng': '您的身份已认证',
  'mt.nindezhengjianrenzheng': '您的证件认证',
  'mt.nindezhengjianyirenzheng': '您的证件已认证',
  'mt.zhanghao': '账号',
  'mt.shenfenxinxi': '身份信息',
  'mt.zhengjianleixing': '证件类型',
  'mt.zhengjianhaoma': '证件号码',
  'mt.YourPasswordMustContain': '您的密碼必須包含：',
  'mt.ContainsatLeastOneNumber': '至少包含一個數字',
  'mt.PasswordlengthIs8To16Characters': '密碼長度為8到16個字符',
  'mt.ContainsuppercaseAndLowercaseCharacters': '包含大寫和小寫字符',
  'mt.ContainsAtLeastOneSpecialCharacter': '至少包含一個特殊字符，如(!@#$%)',
  'mt.yongjin': '佣金',
  'mt.myMessage': '我的消息',
  'mt.shijianbunengdayusangeyue': '時間不能大於90天',
  'mt.timeTips1': '開始時間不能大於結束時間',
  'mt.timeTips2': '結束時間不能小於開始時間',
  'mt.shenfenrenzhengweitongguotips': '身份認證審核未通過，請重新進行認證或者聯繫客服',
  'mt.womenhuijinkuaiwanchengshenhe': '我們會盡快審核您的身份認證，請耐心等待, 如有疑問請聯繋客服。',
  'mt.chongxinrenzheng': '重新认证',
  'mt.lianxikefu': '联系客服',
  'mt.xiugaimima': '修改密码',
  'mt.yanzhengshiyudaowenti': '验证时遇到问题',
  'mt.xinmima': '新密码',
  'mt.shuruxinmima': '输入新密码',
  'mt.querenxinmima': '确认新密码',
  'mt.zaicishuruxinmima': '再次输入新密码',
  'mt.yuanshiyouxiangyanzheng': '原始邮箱验证',
  'mt.codeDownload': '{count}秒後重新獲取',
  'mt.xinmimashurubuyizhi': '新密码输入不一致',
  'mt.shuruyouxiangyanzhengma': '输入邮箱验证码',
  'mt.shoujiyanzheng': '手机验证',
  'mt.shuruduanxinyanzhengma': '输入短信验证码',
  'mt.genggaidianhuahaoma': '更改电话号码',
  'mt.xinshoujihaoma': '新手机号码',
  'mt.xinshoujiyanzheng': '新手机验证',
  'mt.yuanshishoujiyanzheng': '原始手机验证',
  'mt.genghuandianziyouxiang': '更改电子邮箱',
  'mt.xinyouxiang': '新电子邮箱',
  'mt.xinyouxiangyanzheng': '新邮箱验证',

  'mt.bangdingshouji': '绑定手机',
  'mt.bangdingyouxiang': '绑定邮箱',
  'mt.yongyuyanzhengnidezhanghu': '用于验证你的账户和未来的操作',
  'mt.shurunindeshoujihaoma': '请输入您的手机号码',
  'mt.qingshurunindeyouxiang': '请输入您的邮箱',
  'mt.qingshurushoujihao': '请输入手机号',
  'mt.qingfasongyanzhengmahuoqu': '请重新发送验证码获取',
  'mt.shenfenrenzheng': '身份认证',
  'mt.pingzhengrenzheng': '凭证认证',
  'mt.shenfenrenzhengtips': '請確保以下資訊是本人名下最新且有效的身份资讯',
  'mt.xing': '姓',
  'mt.ming': '名',
  'mt.shuruxingshi': '输入姓氏',
  'mt.shurumingzi': '输入名字',
  'mt.xuanzezhengjianleixing': '选择证件类型',
  'mt.zhengjianhao': '证件号',
  'mt.shurunindezhengjianhaoma': '输入您的证件号码',
  'mt.pingzhengrenzhengtips': '上傅的照片必缜能看清证件信息，确保照片内容真实有效，不得做任何修改',

  'mt.shangchuanzhengjian': '上传证件',
  'mt.dianjishangchuantupian': '点击或拖动图片到此处',

  'mt.shenhezhong': '审核中',
  'mt.kycrenzhengyiwancheng': 'KYC认证完成',
  'mt.shenfenrenzhengshenhezhong': '身份认证审核中',
  'mt.shenfenrenzhengshenhezhongtips': '我們會盡快完成審核，請您耐心等待如有疑問，請聯繋客服',

  // 新加的字段 @TODO
  'mt.kycUploadTips': '支持JPG/JPEG/PNG,不能超过1M',
  'mt.kycUploadImgSizeTips': '图片大小不能超过{size}',
  'mt.genggai': '更改',
  // ======= 个人中心-设置 结束 ==========

  // ========== 个人中心-跟单 开始 ==========
  'mt.gendanguangchang': '跟单广场',
  'mt.gendanguanli': '跟单管理',
  'mt.daidanguanli': '带单管理',
  'mt.qingshuruxinyouxiang': '请输入新邮箱',
  'mt.qingshuruxinshouji': '请输入新手机',
  'mt.nindesshenfenrenzheng': '您的身份认证',
  'mt.tijiaochenggong': '提交成功',
  'mt.qingshuruyouxiang': '请输入邮箱',
  'mt.bangdingshoujihuoyouxiang': '绑定手机/邮箱',
  'mt.duo': '多',
  'mt.kong': '空',
  'mt.lijicanjia': '立即參加',
  'mt.zhanghuleixing': '賬戶類型',
  'mt.biaozhun': '標準',
  'mt.luodian': '裸點',
  'mt.meifen': '美分',
  'mt.biaoqian': '標籤',
  'mt.dipin': '低頻',
  'mt.jinqishouyilv': '近{range}收益率',
  'mt.jin': '近',
  'mt.14': '兩週',
  'mt.liangzhou': '兩週',
  'mt.30': '一月',
  'mt.yiyue': '一月',
  'mt.yigeyue': '一個月',
  'mt.lianggeyue': '兩個月',
  'mt.yigendan': '已跟單',
  'mt.gendan': '跟單',
  'mt.jieshugendan': '結束跟單',
  'mt.jieshugendanzhong': '結束跟單中',
  'mt.jieshudaidanzhong': '結束帶單中',
  'mt.wufagendan': '無法跟單',
  'mt.yimanyuan': '已滿員',
  'mt.jinqiyingkui': '{range}盈虧',
  'mt.quanbuyingkui': '全部盈虧',
  'mt.leijijiaoyibishu': '累計交易筆數',
  'mt.leijigensuirenshu': '累計跟隨人數',
  'mt.jinqishenglv': '{range}勝率',
  'mt.changjianwenti': '常見問題',
  'mt.daidan': '帶單',
  'mt.leijifenrun': '累計分潤',
  'mt.jinrifenrun': '今日分潤',
  'mt.dangqiangensuirenshu': '當前跟隨人數',
  'mt.ruzhutianshu': '入住天數',
  'mt.daidanbaozhengjinyue': '帶單保證金餘額',
  'mt.guanlizichanguimo': '管理資產規模',
  'mt.fenrunbili': '分潤比例',
  'mt.shenqingchengweidaidanyuan': '申請成為帶單員',
  'mt.qugendan': '去跟單',
  'mt.zhanghubaozhengjin': '賬戶保證金',
  'mt.zhanghuyue': '賬戶餘額',
  'mt.leijiyingkui': '累計盈虧',
  'mt.jingyingkui': '淨盈虧',
  'mt.leijigendanrenshu': '累計跟單人數',
  'mt.yonghuyingkui': '用戶盈虧',
  'mt.fenrunlibi': '分潤比例',
  'mt.zichanyaoqiu': '資產要求',
  'mt.zongzhichan': '總資產',
  'mt.daidanbiaoxian': '帶單表現',
  'mt.jiaoyipianhao': '交易偏好',
  'mt.shenglv': '勝率',
  'mt.zongshouyilv': '總收益率',
  'mt.daidanyingkui': '帶單盈虧',
  'mt.gendanyonghuyingkui': '跟單用戶盈虧',
  'mt.huichelv': '回撤率',
  'mt.pingjunmeibishouyilv': '平均每筆收益率',
  'mt.yingkuie': '盈虧額',
  'mt.jiaoyidui': '交易對',
  'mt.jiaoyicishu': '交易次數',
  'mt.dangqiandaidan': '當前帶單',
  'mt.lishidaidan': '歷史帶單',
  'mt.gendanyonghu': '跟單用戶',
  'mt.weishixianyingkui': '未實現盈虧',
  'mt.pinzhong': '品種',
  'mt.chicangshuliang': '持倉數量',
  'mt.gensuirenshu': '跟隨人數',
  'mt.fenrunmingxi': '分潤明細',
  'mt.jieshudaidan': '結束帶單',
  'mt.daidanshuju': '帶單數據',
  'mt.zongzichan': '總資產',
  'mt.jiaoyiyingkui': '交易盈亏',
  'mt.kaicangjunjia': '开仓均价',
  'mt.qiangpinjia': '強平價',
  'mt.pingcangshijian': '平倉時間',
  'mt.kaicangshijian': '開倉時間',
  'mt.zuidachicangliang': '最大持倉量',
  'mt.pingcangjunjia': '平倉均價',
  'mt.pingcangshuliang': '平倉數量',
  'mt.chicangjunjia': '持倉均價',
  'mt.yonghu': '用戶',
  'mt.gendanyingkui': '跟單盈虧',
  'mt.gendanjine': '跟單金額',
  'mt.zuidagendanjine': '最大跟單金額',
  'mt.gensuitianshu': '跟隨天數',
  'mt.days': '{count}天',
  'mt.quanbugendanbaozhengjin': '全部跟單保證金',
  'mt.quanbugendanjine': '全部跟單金額',
  'mt.zhanghuzongzichan': '賬戶總資產',
  'mt.jinxingzhong': '進行中',
  'mt.yijieshu': '已結束',
  'mt.lishicangwei': '歷史倉位',
  'mt.baozhengjinyue': '保證金餘額',
  'mt.yishixianyingkui': '已實現盈虧',
  'mt.fenrunjine': '分潤金額',
  'mt.gensuitianshui': '跟隨天數',
  'mt.gensuikaishishijian': '跟隨開始時間',
  'mt.gensuijieshushijian': '跟隨結束時間',
  'mt.dangqiangendan': '當前跟單',
  'mt.lishigendan': '歷史跟單',
  'mt.daidanjiaoyiyuanshenqing': '帶單交易員申請',
  'mt.liangbuwancheng': '讓我們通過兩步完成帶單申請吧~',
  'mt.daidanzhanghuleixing': '帶單賬戶類型',
  'mt.daidanzhanghu': '帶單賬戶',
  'mt.jieshao': '介紹',
  'mt.heyuejiaoyizhengming': '合約交易證明',
  'mt.heyuejiaoyizhengmingtishi':
    '请上传您>30 日的合约收益曲线、交易数請上傳您>30日的合約收益曲線、交易數據統計等截圖;若不上傳，我們將默認審核您{name}合約交易記錄。 可上傳1~3張国片，單張国片大小不超過10 MB',
  'mt.kexuan': '(可選)',
  'mt.shangchuantupianhuotuozhuaifangru': '上傳圖片或者拖拽放入',
  'mt.tijiaoshenqing': '提交申請',
  'mt.touxiangdaxiao': '注意：头像大小大于500kb，小于2m',
  'mt.qingshangchuantouxiang': '請上傳頭像',
  'mt.yuedubingtingyu': '我已閱讀並同意跟單交易{fuwu}和{yinsi}',
  'mt.fuwutiaokuan': '服務條款',
  'mt.yinsizhengceshengming': '隐私政策声明',
  'mt.fanhuigendan': '返回跟單',
  'mt.daidanshezhi': '帶單設置',
  'mt.jibenxinxi': '基本信息',
  'mt.daidancanshu': '帶單參數',
  'mt.lirunfenchengbili': '利潤分成比例',
  'mt.zichanguimo': '資產規模',
  'mt.zuidazhichirenshu': '最大支持人數',
  'mt.daidanbiaoqian': '帶單標籤',
  'mt.ren': '人',
  'mt.yincangfenrunwei0deriqi': '隱藏分潤為 0 的日期',
  'mt.daidanfenrun': '帶單分潤',
  'mt.fenrun': '分潤',
  'mt.meiyougengduojilu': '沒有更多記錄了...',
  'mt.gudingjine': '固定金額',
  'mt.gudingbili': '固定比例',
  'mt.gendanzhanghu': '跟單賬戶',
  'mt.meibigendanbaozhengjin': '每筆跟單保證金',
  'mt.meibigendanbaozhengjinbili': '每筆跟單保證金比例',
  'mt.gendanpeizhi': '跟單配置',
  'mt.gaojipeizhi': '高級配置',
  'mt.gaojishezhi': '高級設置',
  'mt.woyiyuedubingtongyi': '我已閱讀並同意',
  'mt.qinggouxuan': '請勾選',
  'mt.gendanyonghuxieyi': '《跟單用戶協議》',
  'mt.zaikaolvxia': '再考慮下',
  'mt.jieshugendanhou': '結束跟單後，已跟隨未平倉的合約仍然會參與收益',
  'mt.youyudangqianzhanghuyouchicang': '由於您當前賬戶有持倉，或沒有可用的交易賬戶、交易賬戶正在跟單，是否需要創建新的交易賬戶？',
  'mt.dingdanweichuli': '你有訂單未處理，無法結束帶單，請先平倉',
  'mt.jieshudaidanhou': '結束帶單後，需要再次申請才能成為帶單員',
  'mt.qupingcang': '去平倉',
  'mt.wujiaoyizhanghu': '無交易賬戶',
  'mt.dangqianwujiaoyizhanghu': '您當前無此交易賬戶，是否需要創建？',
  'mt.cdexsshequ': 'CDEX 社區',
  'mt.guanyucdex': '關於 CDEX',
  'mt.gongsijianjie': '公司簡介',
  'mt.boke': '博客',
  'mt.fuwu': '服務',
  'mt.haoyoutuijian': '好友推薦',
  'mt.bangzhuzhongxin': '幫助中心',
  'mt.jiaoyifeiyong': '交易費用',
  'mt.jiangli': '獎勵',
  'mt.xueyuan': '學院',
  'mt.chanpin': '產品',
  'mt.xinshouzhinan': '新手指南',
  'mt.tiaokuan': '條款',
  'mt.fengxiantishi': '風險提示',
  'mt.yinsiquantiaokuan': '隱私權條款',
  'mt.saomajiaruwomen': '掃碼加入我們',
  'mt.chuangjianxinjiaoyizhanghu': '創建新交易賬戶',
  'mt.youyuningdangqianzhanghuyouchicang': '由于您当前账户有持仓，或没有可用的交易账户、交易账户正在跟单，是否需要创建新的交易账户？',
  'mt.zidongjiechugendanguanxi': '當跟單該交易員淨{type}達到{value}時，將自動解除跟單關係',
  'mt.sunshi': '損失',
  'mt.yingli': '盈利',
  'mt.jingzichan': '淨資產',
  'mt.xiangqing': '詳情',
  'mt.chengjiaojiage': '成交價格',
  'mt.caozuochenggong': '操作成功',
  'mt.caozuoshibai': '操作失敗',
  'mt.qingxianzhizaishigezifuyinei': '請限制在十個字符以內',
  'mt.qingxianzhizailiangbaizifuyinei': '請限制在两百個字符以內',
  // 'mt.daidanzhuangtai3': '已跟单',
  'mt.daidanzhuangtai2': '已滿員',
  'mt.daidanzhuangtai1': '可跟單',
  'mt.daidanzhuangtai0': '我的带单',
  'mt.xitongcuowu': '系統錯誤',
  'mt.dayuxianzhifanwei': '不得大於{type}限制範圍：{value}',
  'mt.xiaoyuxianzhifanwei': '不得小於{type}限制範圍：{value}',

  'mt.daidanzhuangtai3': '可跟單',
  'mt.chaochuxianzhifanwei': '超出{type}限制範圍：{value}',
  'mt.yibeijinyong': '已被禁用',
  'mt.zhanneixin': '站内信',
  'mt.quanbuyidu': '全部已读',
  //  ==========个人中心-跟单 结束 ==========

  //  ========== 其他组件相关 开始 ==========
  'mt.uploadHelpTips': '点击上传图标，支持png/jpg/svg，尺寸125x125，大小不超过300KB',
  'mt.uploadFileTypeTips': '只能上传格式为png/jpeg/jpg/svg的文件',
  'mt.wenjiandaxiaobunengchaoguo': '文件大小不能超過 {limit}MB',
  // ========== 其他组件相关 结束 ==========

  // 20250212
  'mt.kyczhuangtai': 'KYC 狀態',
  'mt.xinxi': '信息',
  'mt.zhanghuguanlitips2': '用於登錄{name}的輔助信息。注意自己的信息安全。',
  'mt.anquan': '安全',
  'mt.chakanxinxi': '查看信息',
  'mt.weiwanchengchujirenzheng': '未完成初級認證',
  'mt.quchujirenzheng': '去初級認證',
  'mt.yiwanchengchujirenzheng': '已完成初級認證',
  'mt.gaojirenzhengbeijujue': '高級認證被拒絕',
  'mt.zaishiyici': '再試一次',
  'mt.qugaojirenzheng': '去高級認證',
  'mt.yiwanchenggaojirenzheng': '已完成高級認證',
  'mt.qingnaixindenghoushenhe': '請耐心等候審核',
  'mt.qingwanchengchujirenzheng': '請完成初級認證，以便入金進行交易',
  'mt.weifangbianchujin': '為方便出金，建議您完成高級認證',
  'mt.qingzixianzhaoshuomingchongshi': '請仔細按照說明重試',
  'mt.ninyiwanchengsuoyourenzheng': '您已完成所有認證，開始交易吧',
  'mt.kycrenzheng': 'Kyc 認證',
  'mt.wanchengchujirenzheng': '完成初級認證',
  'mt.kycjichurenzheng': 'Kyc 基礎認證',
  'mt.kycgaojirenzheng': 'Kyc 高級認證',
  'mt.dengdairujin': '等待入金',
  'mt.rujinjine': '入金金額',
  'mt.cankaohuilv': '參考匯率',
  'mt.rujinzhanghu': '入金賬戶',
  'mt.fukuanxinxi': '付款信息',
  'mt.shoukuanxingming': '收款姓名',
  'mt.shoukuanyinhang': '收款銀行',
  'mt.daizhifujine': '待支付金額',
  'mt.youyuhuilvbodong': '由於匯率波動，請盡快支付，實際支付金額以提交訂單為準',
  'mt.quxiaodingdan': '取消訂單',
  'mt.yifukuanshagnchuanpingzheng': '已付款成功，請上傳付款憑證',
  'mt.qingzaishijianneiwanchengdingdanzhifu': '請在{time}內完成訂單支付，過期將自動取消訂單',
  'mt.shangchuanpinzheng': '上傳憑證',
  'mt.qingshangchuangpingzheng': '請上傳憑證',
  'mt.nindezijinyujiliangxiaoshineidaoda': '您的資金預計兩小時內到達',
  'mt.nindejiaoyizhengzaichulizhong': '您的資金正在處理中',
  'mt.rujinshenqingchenggong': '入金申請成功，客服正在加緊審核中...',
  'mt.chakandingdan': '查看訂單',
  'mt.rujinshiyudaowenti': '入金時遇到問題？',
  'mt.shoukuanerweima': '收款二維碼',
  'mt.weixinsaomazhifu': '微信掃碼支付，請確保收款商家信息一致',
  'mt.zhifubaosaomazhifu': '支付寶掃碼支付，請確保收款商家信息一致',
  'mt.beizhuzijiderujinzhanghu': '備註自己的入金賬戶，注意不要備註任何違規信息',
  'mt.tianjiayinhangka': '添加銀行卡',
  'mt.yinhangmingcheng': '銀行名稱',
  'mt.qingshuruyinhangmincheng': '請輸入/選擇銀行名稱',
  'mt.qingshuruyinhangmingcheng': '請輸入/選擇銀行賬戶',
  'mt.pingtaihuilv': '平台匯率',
  'mt.zijinjilu': '資金記錄',
  'mt.kefu': '客服',
  'mt.yinhangkadizhi': '銀行卡地址',
  'mt.qianbaodizhi': '錢包地址',
  'mt.querentixian': '確認提現',
  'mt.shoukuanfangshi': '收款方式',
  'mt.anquanrenzheng': '安全認證',
  'mt.dingdanshixiao': '訂單失效，請重新創建',
  'mt.chakanduanxin': '查看短信,獲取驗證碼',
  'mt.yanzhengmajiangfasongzhi': '驗證碼將發送至{value}，請查看您的{type}。',
  'mt.shoujiduanxin': '手機短信',
  'mt.huoquyanzhengma': '獲取驗證碼',
  'mt.qingshuruzhanghaomimayanzhengma': '請輸入賬號密碼和驗證碼',
  'mt.gerenxinxi': '个人信息',
  'mt.zhuceriqi': '注册日期',
  'mt.guojia': '国家',
  'mt.shoujihao': '手机号',
  'mt.youxiang': '邮箱',
  'mt.bangding': '绑定',
  'mt.weibangding': '未绑定',
  'mt.mima': '密码',
  'mt.dizhi': '地址',
  'mt.xxtiao': '{count}条',
  'mt.xiuggaimima': '修改密码',
  'mt.xiuggaimimatishi': '更换成功24小时内您无法进行出金操作',
  'mt.qurenxinmima': '确认新密码',
  'mt.shoujiyanzhengma': '手机验证码',
  'mt.shurunindexinmima': '输入您的新密码',
  'mt.shurushoujiyanzhengma': '输入手机验证码',
  'mt.mimabuyizhi': '密码不一致',
  'mt.qingtianxiequerenmima': '请填写确认密码',
  'mt.xiugaidizhi': '修改地址',
  'mt.youxiangyanzhengmafasongzhitips': '验证码将发送至{email}。请查看您的邮箱或垃圾桶',
  'mt.quhao': '区号',
  'mt.chujinxianzhi': '出金限值',
  'mt.xuanzeshijianfanwei': '選擇時間範圍',
  'mt.dakuanwangluo': '付款網絡',
  'mt.fukuanzhanghu': '付款賬戶',
  'mt.dengdaichujin': '等待出金',
  'mt.nindejiaoyizhengzaichulizhong2': '您的交易正在處理中',
  'mt.chujinshenqingchenggong': '出金申請成功，客服正在加緊審核中...',
  'mt.zijinyujiliangxiaoshineidaoda': '您的資金預計兩小時內到達',
  'mt.qujiaoyi': '去交易',
  'mt.xuanzerujinfangshi': '選擇入金方式',
  'mt.shengyufukuanshijian': '剩餘付款時間 {time}',
  'mt.qingquebaoninbinweijinxingzhuanzhang': '請確保您並未進行轉賬，以免資金損失。',
  'mt.duociquxiao': '多次取消入金後當天無法再進行入金。',
  'mt.rujindengdai': '入金等待',
  'mt.chujindengdai': '出金等待',
  'mt.dingdanyichaoshi': '訂單已超時',
  'mt.zhuceshijian': '註冊時間',
  'mt.gerenziliao': '個人資料',
  'mt.shangchuanpingzheng': '上傳凭證',
  'mt.zhuanrujine': '轉入金額',
  'mt.daozhangjine': '到賬金額',
  'mt.shenfenrenzhengshibai': '身份認證失敗',
  'mt.nintijiaodeziliaobuquanqieyoucuowu': '您提交的資料不完整或存在錯誤，請檢查後重新提交',
  'mt.yicunzaiweiwanchengdingdan': '已存在未完成的訂單，請先完成訂單',
  'mt.ninyiwanchengchujirenzheng': '您已完成初級認證，可以選擇入金或者繼續高級認證（開通出金權限）',
  'mt.renzhengxingming': '認證姓名',
  'mt.chukanshili': '查看範例',
  'mt.quanbuchujinfangshi': '全部出金方式',
  'mt.jiazaigengduo': '加載更多',
  'mt.shezhixinxifubiaoti': '您的聯繫信息將用於賬號管理、接收通知，請及時綁定以免錯過重要通知。',
  'mt.anquanfubiaoti': '如您認為密碼可能已經洩露，請及時更改密碼。',
  'mt.bangdingyouxiangtishi': '綁定您的郵箱將用來接收通知信息。',
  'mt.bangdingshoujitishi': '綁定您的手機將用來接收通知信息。',
  'mt.bunengdayuxxm': '不能大於{size}M',
  'mt.dianjicichujinxinshangchuan': '點擊此處進行上傳'
}
