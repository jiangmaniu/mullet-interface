...
import { AccountTag } from '@/pages/admin/copyTrading/comp/AccountTag'
...
function AccountList() {
  ...
  const renderCardList = () => {
    return (
      <div className={cn('flex items-center overflow-x-auto overflow-y-hidden flex-nowrap pb-10', className)}>
        <div className="flex items-center gap-x-5">
          {(currentAccountList || []).map((item, idx) => {
            ...
            return (
              <div
                className={cn(
                  'w-[360px] h-[344px] border-[2px] rounded-lg px-[26px] relative cursor-pointer bg-brand-secondary-1',
                  isActive ? 'border-brand' : 'border-transparent'
                )}
                // style={{
                //   background: isActive ? `linear-gradient(180deg, #DCECFF 0%, #FFFFFF 40%)` : ''
                // }}
                key={idx}
                onClick={() => {
                  setCurrent(item)
              ...
              >
                ...
                <div>
                  ...
                  <div className=" flex gap-2 items-center">
                    {' '}
                    <div className="text-content-3 text-sm line-clamp-2 break-all">{synopsis?.remark}</div>
                ...
                <div>
                  {(synopsis?.list || []).slice(0, 3).map((v, index) => (
                    <div className="flex items-center justify-between pb-7" key={index}>
                      <div className="text-base text-content-3 truncate w-[140px]">{v.title}</div>
