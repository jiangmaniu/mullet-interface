...
export const fundingFlowTablecolumns: ColumnDef<any>[] = [
  {
    accessorKey: 'id',
...
export function FundingFlow() {
  const table = useReactTable({
    data: MOCK_FUNDING_FLOWS,
    columns: fundingFlowTablecolumns,
  ...
  return (
    <>
      <Table>
        ...
        <TableBody>
          {table.getRowModel().rows?.length ? (
            ...
            <TableRow>
              <TableCell colSpan={fundingFlowTablecolumns.length} className="h-24 text-center">
                No results.
              </TableCell>
            </TableRow>
          )}
      ...
      <div className="flex-1 overflow-auto">
          ...
          <tbody>
            {MOCK_FUNDING_FLOWS.map((flow) => (
              <tr key={flow.id} className="border-b border-gray-800 hover:bg-gray-900/30">
                <td className="p-3">
                  <span className={`px-2 py-1 rounded text-xs ${getTypeColor(flow.type)}`}>{getTypeLabel(flow.type)}</span>
                </td>
                <td className="p-3 text-white">{flow.chain}</td>
                <td className="p-3">
                  <span className={flow.amount.startsWith('+') ? 'text-green-500' : 'text-red-500'}>{flow.amount}</span>
                </td>
                <td className="p-3 text-gray-400">{flow.rate}</td>
                <td className="p-3 text-gray-400 text-xs">{flow.timestamp}</td>
                <td className="p-3">
                  <a href={`#${flow.txHash}`} className="text-blue-400 hover:text-blue-300 flex items-center gap-1">
                    {flow.txHash}
                    <span>↗</span>
                  </a>
                </td>
        ...
        {MOCK_FUNDING_FLOWS.length === 0 && <div className="flex items-center justify-center h-40 text-gray-500">暂无资金流水</div>}
      </div>
    </>
  )
}
