...
export const TradeLayout = ({ slots }: TradeLayoutProps) => {
  ...
  const [layouts, setLayouts] = useState<{ [key: string]: Layout[] }>(() => {
    ...
    const tradingviewHeight = Math.max(calculateTradingviewHeight(containerHeight), FIXED_HEIGHTS[TradeLayoutKey.Tradingview])
    const orderbooksHeight = FIXED_HEIGHTS[TradeLayoutKey.Tabs] + FIXED_HEIGHTS[TradeLayoutKey.Overview] + tradingviewHeight
    const actionHeight = orderbooksHeight + FIXED_HEIGHTS[TradeLayoutKey.Position] - FIXED_HEIGHTS[TradeLayoutKey.Account]
  ...
  useEffect(() => {
    ...
    if (hasValidCache.current) {
      ...
      const shouldUseCache =
        cached && Math.abs(cached.containerWidth - containerWidth) < 100 && Math.abs(cached.containerHeight - containerHeight) < 100
