...
const SettingPositionTpSlModalContent = observer(({ positionInfo, onConfirm, onClose }: SettingPositionTpSlModalContentProps) => {
  ...
  const { setSl, setSp, slFlag, spFlag, takeProfit, stopLoss, disabledBtnByCondition } = useTrade({ marketItem: positionInfo })
  ...
  return (
    <Form {...form}>
      <form onSubmit={form.handleSubmit(onSubmit)} className="gap-xl flex flex-col">
          ...
          <div className="gap-2xl flex flex-col">
            <div className="gap-medium flex flex-col items-center">
              ...
              <div className="text-paragraph-p1 flex gap-1">
                ...
                <div className={cn(isPositionDirectionBuy ? 'text-market-rise' : 'text-market-fall')}>{buySellInfo.buySellText}</div>
                <div className={cn(isPositionDirectionBuy ? 'text-market-rise' : 'text-market-fall')}>{buySellInfo?.leverageText}</div>
              </div>
            </div>
            <div className="gap-2xl flex flex-col">
              ...
              <div className="flex flex-col gap-2xl">
                <SetTakeProfit positionInfo={positionInfo} />
                <SetStopLoss positionInfo={positionInfo} />
              </div>
            </div>
          </div>
        </div>
        ...
        <ModalFooter>
          <Button block color="primary" type="submit" size="md" loading={form.formState.isSubmitting} disabled={disabledBtnByCondition}>
            <Trans>确定</Trans>
          </Button>
        </ModalFooter>
      </form>
...
const SetTakeProfit = observer(({ positionInfo }: { positionInfo: IPositionItem }) => {
  let {
    disabledTrade,
    spValue,
    sp_scope,
  ...
  useEffect(() => {
    ...
    if (prevIsBuyRef.current !== isBuy && tpPercent && sp_scope) {
      const price = getTakeProfitPrice(tpPercent, isBuy, sp_scope, positionInfo.symbolDecimal)
      setSp(price)
    }
    // 更新 ref 为当前值
  ...
  return (
    <div className="flex flex-col gap-medium">
      <div className={'gap-xl flex-1 flex items-center'}>
        ...
        <NumberInput
          ...
          onValueChange={({ value, floatValue }, { source }) => {
            if (source === NumberInputSourceType.EVENT) {
              setTpPercent(value)
              const price = getTakeProfitPrice(value, isBuy, sp_scope, positionInfo.symbolDecimal)
              setSp(price)
            }
          }}
          min={0}
      ...
      <div>
        <SetTakeProfitLabel positionInfo={positionInfo} />
      </div>
    </div>
  )
})

const SetTakeProfitLabel = observer(({ positionInfo }: { positionInfo: IPositionItem }) => {
...
const SetStopLoss = observer(({ positionInfo }: { positionInfo: IPositionItem }) => {
  ...
  useEffect(() => {
    ...
    if (prevIsBuyRef.current !== isBuy && slPercent && sl_scope) {
      const price = getStopLossPrice(slPercent, isBuy, sl_scope, positionInfo.symbolDecimal)
      setSl(price)
    }
    // 更新 ref 为当前值
    prevIsBuyRef.current = isBuy
    ...
    <div className="flex flex-col gap-medium">
      <div className={'gap-xl flex-1 flex items-center'}>
        <div className="flex-1">
          <NumberInput
            min={0}
            decimalScale={positionInfo.symbolDecimal}
            value={slValue}
        ...
        <div className={'w-[80px]'}>
          <NumberInput
            ...
            onValueChange={({ value, floatValue }, { source }) => {
              if (source === NumberInputSourceType.EVENT) {
                setSlPercent(value)
                const price = getStopLossPrice(value, isBuy, sl_scope, positionInfo.symbolDecimal)
      ...
      <div>
        <SetStopLossLabel positionInfo={positionInfo} />
      </div>
    </div>
  )
})
const SetStopLossLabel = observer(({ positionInfo }: { positionInfo: IPositionItem }) => {
