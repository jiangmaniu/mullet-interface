...
export function WalletCard({ account, isActive, isCreateNew, onCreateNew }: WalletCardProps) {
  ...
  return (
    <div
      className={`bg-gray-900 border rounded-lg p-6 ${
        isActive ? 'border-blue-500 ring-2 ring-blue-500 ring-opacity-50' : 'border-gray-700'
      }`}
    >
      <div className="space-y-4">
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h3 className="font-semibold text-white">{getWalletDisplayName(account)}</h3>
              <p className="text-sm text-gray-400">{formatAddress(account.address)}</p>
            </div>
          </div>
        </div>
        <div className="flex items-center justify-between">
          <Badge variant={account.chainType === 'solana' ? 'default' : 'secondary'}>{account.chainType.toUpperCase()}</Badge>
