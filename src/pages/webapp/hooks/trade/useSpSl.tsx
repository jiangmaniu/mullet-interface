...
import { formatNum, toNegativeOrEmpty } from '@/utils'
...
export default function useSpSl() {
  ...
  const slProfit = useMemo(() => {
    ...
    if (isPosition || isLimit || isStopLossLimit) {
      return price && sl ? Number((Math.abs(sl - price) * count * consize).toFixed(d)) : 0
    } else if (orderType === 'MARKET_ORDER') {
      return sl ? Number((Math.abs(sl - currentPrice) * count * consize).toFixed(d)) : 0
    }
    // 转换汇率
  ...
  const spProfit = useMemo(() => {
    ...
    // Math.abs 价格和行情的差值，取正数
    if (isPosition || isLimit || isStopLossLimit) {
      return price && sp ? Number((Math.abs(sp - price) * count * consize).toFixed(d)) : 0
    } else if (orderType === 'MARKET_ORDER') {
      return sp ? Number((Math.abs(sp - currentPrice) * count * consize).toFixed(d)) : 0
    }
    // 转换汇率
    profit = calcExchangeRate({
      value: profit,
      ...
      () => {
        if (spPriceOrAmountType === 'AMOUNT' && spAmount) {
          ...
          if (isBuy) {
            ...
            return Number((_price + Number(spAmount || 0) / count / consize).toFixed(d))
          } else {
      ...
      () => {
        if (slPriceOrAmountType === 'AMOUNT' && slAmount) {
          ...
          if (isBuy) {
            ...
            return Number((_price - Number(slAmount || 0) / count / consize).toFixed(d))
          } else {
