import { FormattedMessage } from '@umijs/max'
import dayjs from 'dayjs'

import Iconfont from '@/components/Base/Iconfont'
import { formatNum } from '@/utils'

export default ({ item, onClick }: { item: any; onClick: (item: any) => void }) => {
  return (
    <div className=" rounded-xl bg-white h-16 border border-gray-150 opacity-60 pl-5 pr-4 py-3 flex flex-row justify-between gap-1 w-full items-center">
      <div className="flex flex-row items-center gap-[4rem]">
        <span className=" text-sm font-medium text-primary">{item.date || dayjs().format('YYYY-MM-DD')}</span>

        <div className="flex flex-col items-start gap-1">
          <span className=" text-sm font-medium text-primary !font-dingpro-medium">{formatNum(item.profitSharingAmountTotal)}</span>
          <span className=" text-xs text-gray-500">
            <FormattedMessage id="mt.daidanfenrun" />
          </span>
        </div>
        <div className="flex flex-col items-start gap-1">
          <span className=" text-sm font-medium text-primary">{item.followerNumber}</span>
          <span className=" text-xs text-gray-500">
            {' '}
            <FormattedMessage id="mt.gensuirenshu" />
          </span>
        </div>
      </div>
      <div
        onClick={() => {
          onClick(item)
        }}
        className="w-7 h-7  rounded-lg bg-white flex items-center justify-center border border-solid border-gray-150 cursor-pointer hover:shadow-[2px_2px_2px_rgba(100,100,100,0.25)]"
      >
        <Iconfont name="gengduo-caozuo" width={14} color="black" height={14} />
      </div>
    </div>
  )
}
